...
#    my_file = os.popen(fileurl).read().strip('\n')
...
#    print(type(s))
##    my_file.seek(0,0)
#    my_file.write(s.strip('\n'))
#    
#    lines = open(fileurl,'rb').readlines()  #打开文件，读入每一行
#    for s in lines:
#        my_file.write(s.replace('\n','NA'))    # replace是替换，write是写入
##    my_file.close()  # 关闭文件
...
#    print('改名成功')
    ...
    #把数据进行第一次分列
    data_split = pd.DataFrame((x.split('] {') for x in data.logname),index=data.index,columns=['createtimeandtype','Remark'])
#    print(data_split)
    
    data_split2 = pd.DataFrame((x.split('] [') for x in data_split.createtimeandtype),index=data.index,columns=['createtime','type'])

    #单独生成createtime列
    data_createtime=data_split2['createtime'].str.replace('[','')
    
    #单独生成Type列
    data_type=data_split2['type']
...
#   根据文件大小处理
    ...
    fileread(r'D:\company\日志\logtest\cocohub.20180321.175639.714.log','Total')
